...
5. **Book**
   ...
   - Optional header **`Idempotency-Key`**: non-empty string, max 128 chars. Safe retries return the **same JSON body and HTTP status** as the first successful request for that key + body; the same key with a **different** body returns **409**.
