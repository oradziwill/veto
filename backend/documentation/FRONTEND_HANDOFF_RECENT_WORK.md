...
**Recent consumables (`GET …/patients/<id>/recent-supply-lines/`):** staff / vet (same idea as `last-vitals`). Suggestions from past **invoice lines** linked to **inventory** for this patient in this clinic (deduped by item, newest invoice first). **Cancelled** invoices are excluded. Query: `limit` (default 20, max 50). Each element: `inventory_item_id`, `name`, `sku`, `unit`, `last_quantity`, `last_used_at`, `invoice_id`. Read-only — does not create invoices or move stock.
