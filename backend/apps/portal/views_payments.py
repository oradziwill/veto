...
def _portal_complete_deposit_core(
    request,
    invoice_id: int,
    p: PortalPrincipal,
    appt: Appointment,
    ...
    """Inner POST handler for complete-deposit (runs inside idempotency transaction when key set)."""
    if stripe_session_id:
        ...
        if inv.status == Invoice.Status.PAID:
            if Payment.objects.filter(invoice_id=inv.id, reference=ref).exists():
                ...
                return Response(portal_appointment_booking_payload(appt_done, inv_done), status=200)
        ...
        if str(meta.get("invoice_id")) != str(invoice_id):
            return Response({"detail": "Payment session does not match this invoice."}, status=400)
        try:
...
class PortalInvoiceCompleteDepositView(APIView):
    ...
    def post(self, request, invoice_id: int):
        ...
        def handler():
            return _portal_complete_deposit_core(
                request,
                invoice_id,
                p,
                appt,
        ...
        return run_idempotent_portal_post(
            ...
            operation=f"portal_complete_deposit:{invoice_id}",
...
class PortalInvoiceStripeCheckoutView(APIView):
    ...
    def post(self, request, invoice_id: int):
        ...
        return run_idempotent_portal_post(
            ...
            client_id=p.client_id,
            clinic_id=p.portal_clinic_id,
            operation=f"portal_stripe_checkout:{invoice_id}",
            payload_for_hash=payload_for_hash,
            handler=handler,
        )
